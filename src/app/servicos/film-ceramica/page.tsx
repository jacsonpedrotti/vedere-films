...
export default function FilmCeramica() {
  ...
  useEffect(() => {
    if (videoRef.current) {
      ...
      videoRef.current.muted = userPreferences.muted;
      videoRef.current.volume = userPreferences.volume;
      videoRef.current.playbackRate = userPreferences.playbackRate;
  ...
  }, [currentVideoIndex, userPreferences]);
          ...
          <div className="w-full aspect-[16/10] rounded-lg overflow-hidden shadow-2xl relative bg-black">
            ...
            <video 
              ...
              onCanPlay={() => {
                setIsLoading(false);
                if (videoRef.current) {
                  // Aplicar preferências novamente quando o vídeo carregar
                  videoRef.current.muted = userPreferences.muted;
                  videoRef.current.volume = userPreferences.volume;
                  videoRef.current.playbackRate = userPreferences.playbackRate;
            ...
            {videos.map((video, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all cursor-pointer ${
                  currentVideoIndex === index ? 'bg-[#747B7A] w-8' : 'bg-white/50 hover:bg-white/80'
                }`}
